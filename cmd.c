...
int main_exact(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "Ms:")) >= 0) {
		switch (c) {
			case 'M': use_mmap = 1; break;
			case 's': min_occ = atoi(optarg); break;
		}
	}
	if (optind + 2 > argc) {
		fprintf(stderr, "Usage: fermi exact [-M] [-s minOcc] <idxbase.bwt> <src.fa>\n");
		return 1;
	}
	...
	while (kseq_read(seq) >= 0) {
		...
		fm6_smem(e, seq->seq.l, (uint8_t*)seq->seq.s, &a, min_occ);
...
int main_contrast(int argc, char *argv[])
{
	...
	if (optind + 2 > argc) {
		fprintf(stderr, "\nUsage:   fermi contrast <ref.fmd> <query.fmd>\n\n");
	...
	eref = rld_restore(argv[optind+0]);
	eqry = rld_restore(argv[optind+1]);
	sub = fm6_contrast(eref, eqry, k, min_occ, n_threads);
	n_seqs = eqry->mcnt[1];
	rld_destroy(eref);
	rld_destroy(eqry);
